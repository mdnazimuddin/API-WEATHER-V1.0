...
class ForecastService
{
    ...
    func getCurrentWeather(city:String, country:String,completion: @escaping (CurrentWeather,CurrentWeatherModel)->Void){
        if let forecastURL = URL(string: "\(forecastBaseURL!)\(city),\(country)&appid=\(forecastAPIKey)") {
            Alamofire.request(forecastURL).responseJSON { (response) in
                
                if let jsonDictionary = response.result.value as? [String:Any] {
                    let name = CurrentWeatherModel(name: jsonDictionary["name"] as! String)
                    if let currentWeatherDictionary = jsonDictionary["main"] as? [String : Any]{
                        let currentWeather = CurrentWeather(weatherDictionary: currentWeatherDictionary)
                        completion(currentWeather,name)
                    }
                }
            }
        }
